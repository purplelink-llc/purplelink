# Outreach Log

Track every outreach action here. One row per send. Status: `sent` / `replied` / `included` / `declined` / `no-response`.

After 4 weeks with no response, mark `no-response` and move on.

---

## LibGuide outreach

| Date sent | Target | Contact | Status | Notes |
|---|---|---|---|---|
| | UVa — Overleaf guide | Ricky Patterson (ricky@virginia.edu) | | |
| | RMIT — LaTeX/BibTeX | Mike Brooks (mike.brooks@rmit.edu.au) | | |
| | Montclair — Citation Tools | Clair Bair (bairdc@mail.montclair.edu) | | |
| | TSU — Citation Tools | Dr. Xuemei Ge (xge@tnstate.edu) | | |
| | Fresno State — Citation Managers | D. Drexler (ddrexler@csufresno.edu) | | |
| | MIT — LaTeX/BibTeX | (Ask form) | | |
| | Yale — BibTeX | (Ask form) | | |
| | UMass Amherst — LaTeX ref database | (Ask form) | | |

## TeX StackExchange answers

| Date | Question URL | Template used | Score after 7d | Notes |
|---|---|---|---|---|
| | | | | |

## Reddit posts

| Date | Subreddit | Post URL | Template used | Upvotes 7d | Notes |
|---|---|---|---|---|---|
| | | | | | |

## LinkedIn / Bluesky posts

| Date | Platform | Post # / topic | Impressions 7d | Notes |
|---|---|---|---|---|
| | | | | |

## Newsletter pitches

| Date | Target | Editor | Status | Notes |
|---|---|---|---|---|
| | iOS Dev Weekly | Dave Verwer | | |
| | MacStories | Federico Viticci | | |
| | Indie Hackers | (forum post) | | |
| | The Sweet Setup | (when ModernTex ships) | | |
| | Hacker News Show HN | (per launch) | | |
| | Product Hunt | (per launch) | | |

## Identity setup

| Date | Platform | URL | Status |
|---|---|---|---|
| | LinkedIn | linkedin.com/company/... | |
| | GitHub | github.com/purplelink-llc | |
| | Bluesky | bsky.app/profile/purplelink.llc | |

---

## Quarterly review

After 90 days, summarize:

- Total referring domains gained: ___
- TeX SE answer score total: ___
- Newsletter placements: ___
- Top-performing single post: ___
- Highest-ROI activity: ___
- Lowest-ROI activity (consider dropping next quarter): ___
